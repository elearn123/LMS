...
public class CalendarEnteryForm extends HttpServlet
{
    ...
    public void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        response.setContentType(CalendarEnteryForm.CONTENT_TYPE);
    ...
    public void getResult(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final String student_id) throws IOException, ServletException {
        ...
        final Option[] option17 = new Option[12];
        final Option[] option18 = new Option[31];
        final Option[] option19 = new Option[12];
        final Option[] option20 = new Option[31];
        String strbody = "";
        ...
        for (int i = 0; i < month.length; ++i) {
            ...
            option17[i] = new Option(strRow).addElement(month[i]);
            option19[i] = new Option(strRow).addElement(month[i]);
        }
        for (int i = 0; i < days.length; ++i) {
            ...
            option18[i] = new Option(strRow2).addElement(days[i]);
            option20[i] = new Option(strRow2).addElement(days[i]);
        }
        ...
        if (vPersonalDataList == null) {
            CalendarEnteryForm.log.entry("getResult()");
            CalendarEnteryForm.log.info("This is Ok no Enevt");
        }
        else {
            System.out.println("vPersonalDataList--------------------" + vPersonalDataList);
            ...
            final String year11 = strEnddate.substring(0, 4);
            final String month2 = strEnddate.substring(5, 7);
            final String day11 = strEnddate.substring(8, 10);
            final String strEnddate2 = year11 + month2 + day11;
            System.out.println("year11,month11,day11,strEnddate2----------------------" + year11 + " , " + month2 + " , " + day11 + " ," + strEnddate2);
            if (strEnddate2 != null) {
                System.out.println("strEnddate---------------" + strEnddate2);
                present2 = sd2.parseDate(strEnddate2);
            }
            final String year2 = strEnddate.substring(0, 4);
            final String month3 = strEnddate.substring(5, 7);
            final String dat22 = strEnddate.substring(8, 10);
            System.out.println("year-month22-dat22------------------------------------------" + year2 + " , " + month3 + " , " + dat22);
            final int setYear2 = sd.getYear(present2);
            final int setMonth2 = sd.getMonth(present2);
            final int setDate2 = sd.getDays(present2);
            System.out.println("setYear2,setMonth2,setDate2-------------" + setYear2 + "  , " + setMonth2 + "  , " + setDate2);
            option17[setMonth2].setSelected(true);
            option18[setDate2 - 1].setSelected(true);
            option19[setMonth2].setSelected(true);
            option20[setDate2 - 1].setSelected(true);
        ...
        tblenddate.addElement((Element)new CalTRExtension().addRowUserForNoReminder("enddate", "no", "No Enddate")).addElement((Element)new CalTRExtension().addRowUserForUntil("enddate", "yes", "until", "umonth", option19, "uday", option20, btnYear));
        final String javascript = "\n function load(repeat , reminder) {\n   if(repeat =='D') {\n\tdocument.frm.repeat[1].checked=true;\n\t} else if(repeat =='M') {\n\t\tdocument.frm.repeat[2].checked=true;\n\t} else {\n\t\tdocument.frm.repeat[0].checked=true;\n \t}\n\tdocument.frm.enddate[1].checked=true;\n\t}\n\tfunction validate(){\n\t\tif(!fnCheckNull(document.frm.calendartext.value,\"Calendar body Name\")){\n\t\t\tdocument.frm.calendartext.focus();\n\t\t\treturn false;\n\t\t}\n\t\treturn true;\n\t}\n\tfunction validateeventid(){\n\t\tif(!fnCheckNull(document.frm.eventid.value,\"Calendar couldnot modify or delete\")){\n\t\t\tdocument.frm.calendartext.focus();\n\t\t\treturn false;\n\t\t}\n\t\treturn true;\n\t}\n\tfunction validateCalName(){\n\t\tif(!fnCheckNull(document.frm.calendar_name.value,\"Calendar Name, Click on Manage Calendar\")){\n\t\t\treturn false;\n\t\t}\n\t\treturn true;\n\t}\n\tfunction checkEntries(){\n\t\tvar filledIn = false;\n\t\tvar i = test();\n\t\tif(i>1) {\n\t\t\tfor (var counter=0; counter<document.frm.checkbox.length; counter++)\n\t\t\t\tif (document.frm.checkbox[counter].checked == true) {\n\t\t\t\t\tindex = counter;\n\t\t\t\t\tfilledIn = true;\n\t\t\t\t}\n\t\t\tif (filledIn == false){\n\t\t\t\talert('You must select at least one file type');\n\t\t\t\treturn false;\n\t\t\t}\n\t\t}\n\t\tif(i==1)  {\n\t\t\tif (document.frm.checkbox.checked == true) {\n\t\t\t\tfilledIn = true;\n\t\t\t}\n\t\t\tif (filledIn == false){\n\t\t\t\talert('You must select at least one file type');\n\t\t\t\treturn false;\n\t\t\t}\n\t\t}\n\t\treturn true;\n\t}\n\n function add_onclick() {\n\t\tif((validate())&&(validateCalName())){\n  \tvar strdate = '" + strdate + "'  " + "\n\t\t\tdocument.frm.method=\"post\";" + "\n\t\t\tdocument.frm.action = \"./calendar.calendar.CalendarEnteryForm?prmAddModify=0&function=dayview&date=\"+strdate;" + "\n\t\t\tdocument.frm.submit();" + "\n\t}" + "\n\t}" + "\n function modify_onclick() {" + "\n\t\tif((validate())&&(validateeventid())){" + "\n  \tvar strdate = '" + strdate + "'" + "\n\t\t\tdocument.frm.method=\"post\";" + "\n\t\t\tdocument.frm.action = \"./calendar.calendar.CalendarEnteryForm?prmAddModify=1&function=dayview&date=\"+strdate;" + "\n\t\t\tdocument.frm.submit();" + "\n\t}" + "\n\t}" + "\n function delete_onclick() {" + "\n\t\tif((validate())&&(validateeventid())){" + "\n  \tvar strdate = '" + strdate + "'  " + "\n\t\t\tdocument.frm.method=\"post\";" + "\n\t\t\tdocument.frm.action = \"./calendar.calendar.CalendarEnteryForm?prmAddModify=2&function=dayview&date=\"+strdate;" + "\n\t\t\tdocument.frm.submit();" + "\n\t}" + "\n\t}" + "\n function close_onclick(viewType ,stryear) {" + "\n var nm = document.frm.calendar_name.value; " + "\n            document.frm.method = \"post\";" + "\n            window.parent.frames[2].document.frm.action = \"./calendar.calendar.MyCalendar?function=\"+viewType+\"&date=\"+stryear+\"&caltype=0&calname=\"+nm;" + "\n            document.frm.submit();" + "\n\t}";
        ...
        tbldata.addElement((Element)new TR().addElement((Element)new TD().setColSpan(2).setAlign("center").addElement((Element)inputButton1).addElement((Element)inputButton2).addElement((Element)inputButton3).addElement((Element)inputButton4)));
    ...
    public void addUser_Calendar(final HttpServletRequest request, final HttpServletResponse response, final String student_id, final PrintWriter out1) throws IOException, ServletException {
        int condition = 0;
        final Vector vPersonalList = CalDataBaseLayer.getPersonalCalendarList("F");
        ...
        if (!strRepeat.equals("N")) {
            ...
            if (strEnddate.equals("yes")) {
                ...
                eDateFormat = null;
                CalendarEnteryForm.log.entry("addUser_Calendar()");
                CalendarEnteryForm.log.info(" eDateFormat" + eDateFormat);
            }
        }
        if (!vPersonalList.contains(strbody)) {
    ...
    static {
        CalendarEnteryForm.CONTENT_TYPE = "text/html";
        log = new SimpleLogger((Class)CalendarEnteryForm.class);
    }
}
