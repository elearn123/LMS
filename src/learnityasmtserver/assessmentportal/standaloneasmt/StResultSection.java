...
public class StResultSection extends HttpServlet
{
    ...
    public void doGet(final HttpServletRequest request, final HttpServletResponse response) throws IOException, ServletException {
        ...
        StResultSection.log.debug("student_id=doget=" + student_id);
        if (student_id != null) {
    ...
    public void getResult(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final String student_id) throws IOException, ServletException {
        ...
        final Vector<Vector> page_questions = allobj;
        for (int i = 0; i < page_questions.size(); ++i) {
            final Vector<Vector> questionMatter = page_questions.elementAt(i);
            ...
            switch (iTitle) {
                ...
                case 4: {
                    ...
                    strQuesNo = (String)question.elementAt(0);
                    StResultSection.log.debug("strQuesNo====mq==========" + strQuesNo);
                    ...
                    vmq.addElement(str3);
                    vmq.addElement(str4);
                    StResultSection.log.debug("str11.length====mq==========" + str3.length);
                    StResultSection.log.debug("str11[0]====mq==========" + str3[0]);
                    vA.addElement(vmq);
                    break;
                }
            }
        ...
        final Doctype doc1 = new Doctype("html", "\"-//W3C//DTD XHTML 1.1 plus MathML 2.0//EN\"", "\"http://www.w3.org/tr/MathML2/dtd/xhtml-math11-f.dtd\"");
        ...
        final font font2 = new font().setColor("#CC0000");
            ...
            for (int j2 = 0; j2 < v.size(); ++j2) {
                ...
                switch (iTitle2) {
                    ...
                    case 4: {
                        ...
                        StResultSection.log.debug("==vMqGivenAns============" + vMqGivenAns2);
                        final String[] leftpAns2 = (String[]) vMqGivenAns2.elementAt(0);
                        final String[] rightAns2 = (String[]) vMqGivenAns2.elementAt(1);
                        iScore = Integer.parseInt((String) res2.elementAt(1));
                        StResultSection.log.debug("===iScore======" + iScore);
                        Marks += iScore;
                        String srtNegscore = "";
                        if (res2.size() > 3) {
                            srtNegscore = (String) res2.elementAt(3);
                            StResultSection.log.debug("===srtNegscore======" + srtNegscore);
                        }
                        final Vector vCorrectAnsmq = (Vector) question2.elementAt(3);
                        StResultSection.log.debug("===vCorrectAnsmq======" + vCorrectAnsmq);
                        boolean b2 = true;
                        boolean c2 = false;
                        for (int r3 = 0; r3 < leftpAns2.length; ++r3) {
                            StResultSection.log.debug("==leftpAns11[r1]============" + leftpAns2[r3]);
                            StResultSection.log.debug("==rightAns11[r1]============" + rightAns2[r3]);
                            ...
                            for (int r4 = 0; r4 < vCorrectAnsmq.size(); ++r4) {
                                final String[] coAns11 = (String[]) vCorrectAnsmq.elementAt(r4);
                                StResultSection.log.debug("==coAns11[0]============" + coAns11[0]);
                                StResultSection.log.debug("==coAns11[1]============" + coAns11[1]);
                            ...
                            StResultSection.log.debug("==========b1=================" + b2);
                        }
                        if (c2) {
                            ++q_attempted;
                            if (b2) {
                        ...
                        StResultSection.log.debug("=========totalScore=================" + totalScore);
    ...
    public div MatchingQuesHTML(final Vector response, final Vector question, final Vector item_responses, final String ans_text, final int no, final String student_id, final String strTime, final String test_id, Double totalScore, int no_correctAns, int no_incorrectAns, Double Marks) {
        ...
        final ReportDataBaseLayer ob1 = new ReportDataBaseLayer();
        ob1.addDetailsQues(student_id, test_id, strTime, strQuesId, strQues, strCorrectAns, ans_text, ans_text, strFeedBack11, cScore, item_score);
        div1.addElement(new div().addElement("" + no).setID("ItemId1")).addElement(new div().addElement(strQues).setID("ItemText1")).addElement(new div().addElement(strCorrectAns).setID("ItemAnswer1")).addElement(new div().addElement(ans_text).setID("AunswerGiven1")).addElement(new div().addElement(cScore).setID("ItemMarks1")).addElement(new div().addElement(strScore).setID("ItemObtainedMarks1")).addElement(new div().addElement(strFeedBack11).setID("Feedback1"));
        return div1;
    }
    ...
    static {
        log = new SimpleLogger((Class)StResultSection.class, true);
    }
}
